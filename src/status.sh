...
Note: prints work link, if present, to stderr by default
EOF
}
...
if [ "$print_link" = "yes" ]; then
  if lib_tl has link; then
    echo "Linked -> $(lib_tl get link)" >&2
  else
    echo "Not linked with anything" >&2
  fi
fi
